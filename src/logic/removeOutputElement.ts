...
export async function removeOutputElement(options) {
    try {
        ...
        output.activeContent = output.activeContent.filter(curr => curr !== `${outputElements.find(el => el.id == options.outputelement).scope == "page" ? pageid : this.config.eventid}-${options.outputelement}`)
        await vyRequest({
